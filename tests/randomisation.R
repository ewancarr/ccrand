...
folder <- "/home/ewan/Downloads/1471-2288-8-65-S1"
covariates <- "4cluster.csv"

random.allocation	<-	function(folder, covariates, unit, outfile, figure) {


area			<-  	folder
stratific		<-	covariates
units			<-  	unit
arms  		<-  	2
...
#######################################################
#  Prelims                          
strat		<-	paste(area,stratific,sep="/")
...
#################################################
# 
covar	<-	read.csv(strat,header=T)
# Name the practices #
rownames(covar)	<-	as.character(covar[,1])
# select first n (units) for practice #
covar			<-	data.frame(covar[1:units,])
# remove practive details
covar			<-	data.frame(covar[,2:ncol(covar)])

### Calculate the standardized Z-scores ###
z     <-    (  covar -  t(matrix(apply(covar,2,mean),ncol(covar),nrow(covar)))  )  / t(matrix(sqrt(apply(covar,2,var)),ncol(covar),nrow(covar)))
...
rm(rand)
gc(TRUE)
colnames(balance)		<-	c(rownames(covar),"balance")
##  Sort the data ##
